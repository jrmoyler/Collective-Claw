...
//  Corporate config
// ─────────────────────────────────────────────────────────────
export const COMPANY_NAME = 'CollectiveClaw';
...
export const DEPARTMENTS: DepartmentConfig[] = [
  ...
  },
  {
    ...
    missions: [
      ...
      'Organize the CollectiveClaw Annual Tech Summit',
...
_agents.push({
  ...
  mission: 'Lead CollectiveClaw to market dominance',
...
export function addAgent(agent: Omit<AgentData, 'index' | 'isPlayer'>, atIndex?: number) {
  const newIndex = atIndex !== undefined ? atIndex : _agents.length;
  ...
  if (atIndex !== undefined && atIndex < _agents.length) {
    _agents[atIndex] = newAgent;
  } else {
